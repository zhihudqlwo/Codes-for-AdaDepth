...
class XCA(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, torch.div(C, self.num_heads, rounding_mode='trunc')) 
...
class LiteMono(nn.Module):
    ...
    def __init__(self, in_chans=3, model='lite-mono', height=192, width=640,
                 ...
                 heads=[8, 8, 8], use_pos_embd_xca=[True, False, False], pretrained=True, **kwargs):

        super().__init__()

        if model == 'lite-mono':
            print(drop_path_rate, "litemono_drop")
    ...
    def load_pretrained_model(self, model_name, dev='cpu'):
        path = f'./ckpt/{model_name}-pretrain.pth' # should be lite-mono-8m-pretrain.pth only
        if not os.path.exists(path):
            print('pretrained ckpt is not found, downloading it now.')
            os.makedirs('./ckpt/', exist_ok=True)
            os.system(f'gdown {self.pretrain_link}; mv lite-mono-8m-pretrain.pth ckpt/')

        model_dict = self.state_dict()
        pretrained_dict = torch.load(path, map_location=dev)['model']
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if (k in model_dict and not k.startswith('norm'))}
        model_dict.update(pretrained_dict)
        self.load_state_dict(model_dict)
        print('pretrained model loaded', model_name)
